помнишь мы сегодня внедрили функцию в наше приложение которое загружает временное приложение при просмотре нашего дополненной реальности функция называется PWa-WebAr , она у нас заработала локально, а почему ты мне кажется что мы её не установили на сервер, Я хочу чтобы ты проверил сервер, где всё есть этой функции или нет если её нет её нужно установить на сервер тоже, посмотри сохранилась ли она у нас локально тоже.
