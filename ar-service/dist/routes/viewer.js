...
router.get('/:id', async (req, res) => {
    try {
        ...
        try {
            const result = await client.query(`SELECT id, status, viewer_html_url, is_demo, expires_at
        FROM ar_projects
        WHERE id = $1`, [id]);
          ...
          `);
            ...
            if (project.status !== 'ready') {
                ...
                if (project.status === 'pending')
                    statusMessage = 'Queued for compilation...';
                else if (project.status === 'error')
                    statusMessage = 'Compilation failed. Please try again.';
