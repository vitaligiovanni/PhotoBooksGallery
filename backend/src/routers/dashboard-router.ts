...
export function createDashboardRouter() {
  ...
  // GET /admin/orders - получить все заказы для админ панели
  router.get("/admin/orders", mockAuth, requireAdmin, async (req, res) => {
    try {
      ...
      console.log(`[admin] GET /admin/orders -> ${allOrders.length} orders fetched`);
      res.json(allOrders);
    } catch (error) {
  ...
  // PUT /admin/orders/:id - обновить статус заказа (админ)
  router.put("/admin/orders/:id", mockAuth, requireAdmin, async (req, res) => {
    try {
      ...
      console.log(`[admin] PUT /admin/orders/${orderId} -> status updated to ${status}`);
