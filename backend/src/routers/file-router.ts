...
export function createFileRouter() {
  ...
  router.get("/public-objects/:filePath(*)", async (req, res) => {
    try {
      const relPath = req.params.filePath;
  ...
  router.get("/objects/:objectPath(*)", async (req, res) => {
    try {
      const relPath = req.params.objectPath;
      const baseDir = path.join(process.cwd(), 'objects');
      const fullPath = path.join(baseDir, relPath);
      ...
      console.error("Error serving object file:", error);
      return res.status(500).json({ error: "Internal server error" });
    }
  });
  ...
  // (Откат) Удалены временные локальные эндпоинты /api/upload и /api/uploads
