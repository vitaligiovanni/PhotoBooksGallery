...
export function createFinanceRouter() {
  ...
  // Admin finance overview endpoint
  router.get('/admin/finances', mockAuth, requireAdmin, async (req: any, res) => {
    try {
  ...
  // Monthly finance stats endpoint
  router.get('/admin/finances/stats/month', mockAuth, requireAdmin, async (req: any, res) => {
    try {
      // Простая заглушка для демо
      const monthlyStats = {
        currentMonth: {
      ...
      console.error("Error fetching monthly finance stats:", error);
      res.status(500).json({ message: "Failed to fetch monthly finance stats" });
    }
  });
